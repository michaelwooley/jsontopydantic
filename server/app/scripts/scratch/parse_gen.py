...
# """
import inspect
from datamodel_code_generator.__main__ import Config, arg_parser
...
include_fields = {
    ...
    "output": False,
    "debug": False,
    "target_python_version": True,
    "base_class": False,
    "custom_template_dir": False,
    "extra_template_data": False,
    "validation": False,
    ...
    "strict_types": True,
    "empty_enum_field_name": True,
    "field_extra_keys": True,
    "field_include_all_keys": True,
    "openapi_scopes": False,
    "wrap_string_literal": False,
    "use_title_as_name": True,
    "http_headers": False,
    "http_ignore_tls": False,
    "use_annotated": True,
    "use_non_positive_negative_number_constrained_types": True,
    "original_field_name_delimiter": True,
    "use_double_quotes": True,
}

# [ m for m in dir(Config) if callable(getattr(Config, m)) and getattr(getattr(Config, m), "__module__", "") == Config.__module__ ]
# "l": inspect.getsourcelines(getattr(Config,m))}
# {m: True   for m in dir(Config) if callable(getattr(Config,m)) and getattr(getattr(Config,m), "__module__","") == Config.__module__}
include_methods = {
    "Config": True,
    "__config__": True,
    "_validate_use_annotated": True,
    "_validate_use_union_operator": True,
    "merge_args": False,
    "validate_file": True,
    "validate_http_headers": True,
    "validate_original_field_name_delimiter": True,
    "validate_path": True,
    "validate_root": True,
    "validate_url": True,
    "validate_use_generic_container_types": True,
}

cs = inspect.getsource(Config)
method_lines = {k: inspect.getsourcelines(getattr(Config, k)) for k in include_methods}
all_method_lines = [l for lines, _ in method_lines.values() for l in lines]

all_lines = [
    (l, any(l.startswith(f"{indent}{nm}:") for nm in Config.__fields__))
    for l in cs.splitlines()
]
model_preamble = "\n".join(l for l, is_field in all_lines if not is_field)

pyd_types = {
    name: (typ if "=" in typ else f"{typ}= None").split("=")
    for name, typ in [
        l.strip().replace(" ", "").split(":") for l, is_field in all_lines if is_field
    ]
}
p_args = {
    a.dest: (a.help or "").replace("'", "\\'")
    for ag in arg_parser._action_groups
    for a in ag._group_actions
}
exclude_lines = []
print(model_preamble)
for name, f in Config.__fields__.items():
    h = p_args.get(name, "")
    td = pyd_types.get(name)
    if td is None:
        raise Exception(f"{name}?!")
    t, d = td
    title = name.replace("_", " ").capitalize()
    include = include_fields.get(name, False)
    include_comment = "" if include else "# "
    line = f"{indent}{include_comment}{name}: {t} = Field(default={d}, title=\"{title}\", description='{h}', advanced=True)"
...
# for name in p_args:
#     if name not in pyd_types:
#         print(name)


# for ag in arg_parser._action_groups:
#     for a in ag._group_actions:
#         f = Config.__fields__.get(a.dest)
#         t = pyd_types.get(a.dest)
#         if f:
#             t = str(f.annotation)
#         else:
#             t = "Any"
#             print(f"SKIPPING: {a.dest}")
#         print(f"{a.dest}: {t} = Field()")
